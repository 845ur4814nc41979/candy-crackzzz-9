...
export default function CartPage() {
  ...
  const { cart, removeFromCart, cartTotal, settings, setOrders, rewardProfiles, setRewardProfiles, staffReferralCodes } = useAppContext();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // If the customer explicitly entered a staff / promo code at checkout,
    // validate it against active standalone codes and use it as the primary
    // employee referral code. If the code is entered but unrecognised, block
    // submission so the customer can correct it.
    ...
    if (trimmedStaffCodeInput) {
      const matchedCode = staffReferralCodes.find(
        c => c.status === 'active' && c.code.toUpperCase() === trimmedStaffCodeInput
      );
      if (matchedCode) {
                  ...
                  <div className="space-y-3 border border-border rounded-2xl p-4 bg-background/60">
                    ...
                    <div className="space-y-1.5">
                      <Input
                        ...
                        onChange={e => {
                          setStaffCodeInput(e.target.value.toUpperCase().replace(/[^A-Z0-9\-_]/g, ''));
                          setStaffCodeError('');
                        }}
                      ...
                      />
                      {staffCodeError && (
                        <p className="text-xs text-destructive font-bold">{staffCodeError}</p>
                      )}
                    </div>
                  </div>
                )}
